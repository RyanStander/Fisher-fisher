...
{
    [SerializeField] float idleSpeed=5f, turnSpeed=40f, switchSeconds=1f, idleRatio=0.3f;
    [SerializeField] Vector2 animSpeedMinMax=new Vector2(0.5f,2f), moveSpeedMinMax = new Vector2(10, 20), changeAnimEveryFromTo = new Vector2(2, 4), changeTargetEveryFromTo = new Vector2(3, 8);
    [SerializeField] Transform homeTarget=null, flyingTarget=null;
    [SerializeField] Vector2 radiusMinMax = new Vector2(5, 20);
    [SerializeField] Vector2 yMinMax = new Vector2(0, 30);
    private bool returnToBase = false;
    [SerializeField] public float randomBaseOffset = 5, delayStart = 0f;

    private Animator animator;
    private Rigidbody body;
    private float changeTarget = 0f, changeAnim = 0f, timeSinceTarget = 0f, timeSinceAnim = 0f, prevAnim = 0f, currentAnim = 0f, prevSpeed = 0f, speed = 0f, zturn = 0f, prevz = 0f,
        turnSpeedBackup = 0f;
    private Vector3 rotateTarget=Vector3.zero, position = Vector3.zero, direction = Vector3.zero, randomizedBase = Vector3.zero;
    private Quaternion lookRotation=new Quaternion(0,0,0,0);
    private float distanceFromBase = 0f, distanceFromTarget = 0f;
    ...
    private Vector3 ChangeDirection(Vector3 currentPosition)
    {
        ...
        else if (distanceFromTarget > radiusMinMax.y&& flyingTarget == null)
        {
            newDir = flyingTarget.position - currentPosition;
        }
        else if (distanceFromTarget < radiusMinMax.x&& flyingTarget == null)
        {
